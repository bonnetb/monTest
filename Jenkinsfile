pipeline {
 ...
 stages {
  ...
  stage('publish') {
   steps {
    sh "mvn deploy -DskipTests=true -DaltDeploymentRepository=nexus::default::http://admin:admin123@nexus3-mon-projet-de-test.apps.sodigital.io/repository/maven-snapshots/ fabric8:build"
   }
  }
